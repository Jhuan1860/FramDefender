...
def welcome_screen():
    game.drawText("Welcome to Farm Defender",360,150,Font(red, 40))
    game.drawText("By Jett Ji & Jennifer Huang",390,180,Font(red, 30))
    game.drawText("Use mouse to aim and left click to shoot",380,230,Font(red))
    game.drawText("Shoot any alien looking thing in gray, not the cow!",380,250,Font(red))
    game.drawText("Press space to start...",380,290,Font(red))
    game.update(30)
    bk.draw()
    game.wait(K_SPACE)

game = Game(1000,600,"Farm Defender")
game.setMusic("sounds/background_music.wav")
...
for l in range(max_level):
    ...
    for x in range(0, 7):
        ...
        invaders[x].setSpeed(random.randint(1,5)*(1+l/100),90)
    ...
    for y in range(0, ceil(l/3)):
        ...
        invaders2[y].setSpeed(random.randint(1,5)*(1+l/100),90)
    ...
    for z in range(0, ceil(l/5)):
        ...
        animals[z].setSpeed(random.randint(1,5)*(1+l/100),90)
